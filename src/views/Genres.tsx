...
export function Genres() {
    ...
    <li key={r.genre}>
      <button
        ...
        onPointerMove={(e) => {
          ...
          if (r.owned === 0 || !nearRowContent(e, ROW_CONTENT)) {
  ...
  // Gelesen-Anzahl einmal berechnen (Zahl und Prozent teilen sich den Wert).
  const popupRead = popupBooks.filter((b) => b.hasRead).length
  const popupCounts =
    popup === null
      ? ''
      : m.views.genres.counts(
          ...
          fmtNum(popupRead),
          fmtNum(popupBooks.length === 0 ? 0 : Math.round((100 * popupRead) / popupBooks.length)),
        )
